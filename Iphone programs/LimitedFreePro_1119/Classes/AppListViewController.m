        ...
        switch(titleAdV._page){
            case 1:     
				if(self.titleAdV.hasPage == NO){
					...
					if(scrollViewFrame.origin.x == 320){
						[UIView beginAnimations:@"Left out" context:nil];
						[UIView setAnimationDuration:1.0];
						[UIView setAnimationDelegate:self];
						[UIView setAnimationDidStopSelector:@selector(moveOutLeftOver)];
...
//	self.navigationItem.title = @"限时免费";
	...
	self._ov = [[OrderView alloc] initWithFrame:CGRectMake(320, 64, 90, 416)];
	[self.navigationController.view addSubview:self._ov];
	[self._ov release];
...
-(void)titleAdViewLoadingOver{
	NSDate *loadOverDate = [NSDate date];
	if(self.refreshDate != nil && [loadOverDate timeIntervalSinceDate:self.refreshDate] > 0){
		NSLog(@"在单元格刷新后加载");
		
		UITableViewCell *section0Cell = [self.tableView cellForRowAtIndexPath:self.sectionIndexPath];
		UIImageView *show = (UIImageView *)[section0Cell.contentView viewWithTag:11];
		...
		if(adScrollViewFrame.origin.x == 320){
